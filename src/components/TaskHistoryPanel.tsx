...
/**
 * TaskHistoryPanel — оптимизированная версия
 *
 * Изменения vs оригинал:
 * 1. Записи истории оборачиваются в VirtualItem —
 *    рендерятся только при попадании во viewport
 * 2. Внешний контейнер наблюдается через IntersectionObserver —
 *    данные загружаются только когда панель раскрыта И видна
 * 3. TaskViewers интегрирован в панель (опционально)
 *
 * Все остальные оптимизации из оригинала сохранены:
 * - module-level constants
 * - React.memo на HistoryEntry, Avatar, FilterTabs
 * - useTimeAgo с одним setInterval
 * - contain: content на каждом entry
 */
...
// ── Sub-компоненты — все из оригинала, без изменений ─────────

const Avatar = memo(function Avatar({ name, isCreation }: { name: string; isCreation: boolean }) {
  ...
  return (
    <div className="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 text-[11px] font-semibold"
      style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.10)', color: 'var(--text-secondary)' }}>
      {initial}
    </div>
  )
})
...
function DiffBadge({ value, variant }: { value: string; variant: 'old' | 'new' }) {
  return (
    <span className="inline-block text-[10px] px-2 py-0.5 rounded-full font-medium max-w-[120px] truncate"
      style={variant === 'old'
...
function DateSeparator({ label }: { label: string }) {
  return (
    <div className="flex items-center gap-2 my-2 ml-9">
      ...
      <span className="text-[9px] font-semibold uppercase tracking-widest whitespace-nowrap px-2 py-0.5 rounded-full"
...
const HistoryEntry = memo(function HistoryEntry({
  entry, timeAgo,
}: { entry: TaskHistory; timeAgo: (s: string) => string }) {
  const { t } = useI18n()
  ...
  if (entry.action_type === 'field_change') {
    ...
    actionLabel = entry.new_value === 'true' ? t('actionCompletedSubtask') : t(cfg.altLabelKey ?? cfg.labelKey)
  } else {
      ...
      <div className="flex gap-2.5">
        ...
        <div className="flex-1 min-w-0" style={{
          ...
          <div className="flex items-baseline gap-1.5 flex-wrap">
            <span className="text-[13px] font-semibold" style={{ color: 'var(--text-primary)' }}>{entry.user.first_name}</span>
            {entry.user.username && <span className="text-[11px]" style={{ color: 'var(--text-dim)' }}>@{entry.user.username}</span>}
            <span className="text-[12px]" style={{ color: 'rgba(62,207,142,0.75)' }}>{actionLabel}</span>
            <span className="text-[10px] ml-auto" style={{ color: 'var(--text-dim)' }}>{timeAgo(entry.created_at)}</span>
          </div>
          {entry.new_value && <p className="text-[11px] mt-0.5 truncate" style={{ color: 'rgba(62,207,142,0.60)' }}>"{entry.new_value}"</p>}
        </div>
      </div>
    )
  }
    ...
    <div className="flex gap-2.5">
      ...
      <div className="flex-1 min-w-0" style={cardBase}>
        <div className="flex items-baseline gap-1.5 flex-wrap mb-1">
          <span className="text-[13px] font-semibold leading-none" style={{ color: 'var(--text-primary)' }}>{entry.user.first_name}</span>
          {entry.user.username && <span className="text-[11px]" style={{ color: 'var(--text-dim)' }}>@{entry.user.username}</span>}
          <span className="text-[12px]" style={{ color: 'var(--text-secondary)' }}>{actionLabel}</span>
          <span className="text-[10px] ml-auto whitespace-nowrap" style={{ color: 'var(--text-dim)' }}>{timeAgo(entry.created_at)}</span>
        </div>
        {isSubtask && subtaskName && entry.action_type !== 'subtask_reordered' && (
          <span className="inline-block text-[11px] px-2 py-0.5 rounded-md max-w-[200px] truncate mb-1"
            style={{
              background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.08)',
              color: entry.action_type === 'subtask_deleted' ? 'var(--text-dim)' : 'var(--text-primary)',
        ...
        {cfg.showDiff && (entry.old_value != null || entry.new_value != null) && (
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            {entry.new_value != null && <DiffBadge value={prettyValue(entry.new_value)} variant="new" />}
          </div>
        )}
      </div>
    </div>
...
}: { active: FilterGroup; onChange: (g: FilterGroup) => void; counts: Record<FilterGroup, number> }) {
  return (
    <div className="flex gap-1.5 overflow-x-auto pb-0.5 mt-3 px-0.5"
      ...
      {FILTER_TABS.map(tab => {
        ...
        return (
          <button key={tab.key} onClick={() => onChange(tab.key)}
            ...
            {tab.label}
            {counts[tab.key] > 0 && (
              <span className="text-[9px] px-1 py-0.5 rounded-full tabular-nums"
                style={isActive
...
export function TaskHistoryPanel({ taskId, userId, initial = [], refetchKey }: Props) {
  ...
  // ── IntersectionObserver на весь контейнер ────────────────
  // Если панель скрыта за нижней границей viewport — данные не грузим
  ...
  useEffect(() => {
    ...
    setFetched(false)
    // Перезагружаем только если панель открыта И в viewport
    if (open && inViewport) {
      void fetchHistory(true)
      setPage(1)
    }
  }, [refetchKey, open, inViewport, fetchHistory])
  ...
  const grouped = useMemo(() => {
    ...
    for (const entry of paginated) {
      ...
      const label = isToday ? t('today') : isYesterday ? t('yesterday')
  ...
  return (
    <div ref={containerRef}>
      ...
      <button
        ...
        style={{
          borderRadius: 16,
          background:   open ? 'rgba(129,115,245,0.09)' : 'rgba(255,255,255,0.04)',
          border: open ? '1px solid rgba(129,115,245,0.28)' : '1px solid rgba(255,255,255,0.08)',
        }}
      >
        ...
        <div className="flex items-center gap-2">
          {open && (
            <button onClick={handleRefresh} className="p-1 rounded-lg"
              style={{ color: refreshing ? 'var(--c-accent)' : 'var(--text-dim)' }} title="Refresh history">
              <RefreshCw size={12} style={{ animation: refreshing ? 'hp-spin 0.7s linear infinite' : 'none' }} />
            </button>
          )}
          {loading
            ? <div className="w-3.5 h-3.5 rounded-full border-2 flex-shrink-0"
                style={{ borderColor: 'rgba(255,255,255,0.12)', borderTopColor: 'var(--c-accent)', animation: 'hp-spin 0.7s linear infinite' }} />
            : open
      ...
      {open && (
        <div className="hp-panel" style={{
          marginTop:           10, borderRadius: 18, padding: '12px 12px 14px',
          ...
          {entries.length > 0 && (
            <FilterTabs active={filterGroup} onChange={g => { setFilterGroup(g); setPage(1) }} counts={counts} />
          )}

          <div style={{ marginTop: 12 }}>
            {filtered.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-8 text-center">
                <span className="text-2xl mb-2">{entries.length === 0 ? '📋' : '🔍'}</span>
                <p className="text-xs" style={{ color: 'var(--text-dim)' }}>
                  {entries.length === 0 ? t('noHistory') : 'No entries for this filter'}
                </p>
                {entries.length === 0 && (
                  <p className="text-[11px] mt-0.5" style={{ color: 'var(--text-dim)' }}>{t('noHistoryDesc')}</p>
                )}
              </div>
            ) : (
              <div className="relative">
                ...
                <div className="space-y-2">
                  {grouped.map(group => (
                    <div key={group.label}>
                      ...
                      {/* ── VirtualItem на каждую запись истории ── */}
                      <div className="space-y-2">
                        {group.items.map((entry, idx) => (
                          <VirtualItem
                            ...
                            rootMargin="80px"
                          >
                            <HistoryEntry entry={entry} timeAgo={timeAgo} />
                          </VirtualItem>
                        ))}
                      </div>
                    </div>
                ...
                {hasMore && (
                  <button
                    ...
                    style={{ color: 'var(--c-accent)', background: 'rgba(129,115,245,0.07)', border: '1px solid rgba(129,115,245,0.18)' }}
                  >
      ...
      <style>{`
        ...
        .hp-panel { animation: hp-panel-in 0.22s cubic-bezier(0.22, 1, 0.36, 1) forwards; }
