...
//       reorderTasks(activeListId!, curr)
//     }
//   }
...
//             </div>
//             {totalCount > 0 && (
//               <p className="text-xs text-text-secondary">{doneCount}/{totalCount} {t('subtasksDone')}</p>
//             )}
//           </div>
...
//                 {searchQuery
//                   ? t('noTasksMatch')
//                   : filter === 'archived' ? t('nothingArchived')
//                   : t('noTasksHere')}
//               </p>
...
//     dueOverdue = diff < 0
//     dueUrgent  = diff <= 1
//     dueLabel   = dueOverdue ? `${Math.abs(diff)}${t('dAgo')}`
//       : diff === 0 ? 'Today'
//       : diff === 1 ? 'Tomorrow'
...
type SortKey        = 'position' | 'due_at' | 'priority' | 'created_at'
type FilterKey      = TaskStatus | 'all' | 'archived'
type ReorderStatus  = 'idle' | 'pending' | 'saving'  // idle → pending (2s) → saving (DB write)
...
export function ListDetailView({ onBack }: Props) {
  ...
  const pendingReorderRef  = useRef<Task[] | null>(null)  // финальный порядок из серии перетаскиваний
  const originalOrderRef   = useRef<Task[] | null>(null)  // исходный порядок для отката
  const wantsToGoBackRef   = useRef(false)                // нажал «назад» во время сохранения?
  ...
  // DnD запрещён если идёт другая операция или сохранение порядка
  ...
  useEffect(() => {
    ...
    pendingReorderRef.current = null; originalOrderRef.current = null
  ...
  useEffect(() => {
    ...
    return () => { supabase.removeChannel(channel); clearTimeout(fetchDebounceRef.current); abortRef.current?.abort() }
  }, [activeListId, user])
  ...
  // ════════════════════════════════════════════════════════════
  //  НАВИГАЦИЯ — блокируется пока идёт сохранение порядка
  // ════════════════════════════════════════════════════════════

  /** Физически выполняет анимацию выхода */
  ...
  function handleBack() {
    if (reorderStatus !== 'idle' || pendingReorderRef.current) {
      // Запоминаем желание выйти — выйдем автоматически после записи
      wantsToGoBackRef.current = true

      // Если всё ещё в фазе debounce — немедленно запускаем сохранение
  ...
  // ════════════════════════════════════════════════════════════
  //  DRAG & DROP — мгновенный UI + дебаунс 2 сек → DB
  // ════════════════════════════════════════════════════════════

  /**
   * handleDragEnd:
   *  1. Мгновенно обновляет UI (reorderTasks)
   *  2. Запоминает финальный порядок в ref
   *  3. Сбрасывает 2-секундный таймер
   *  4. Когда таймер сработает → вызывает flushReorderSave()
   *
   *  Никаких fetch-вызовов здесь нет — всё через дебаунс.
   */
  function handleDragEnd(event: DragEndEvent) {
    if (isViewer || isDndBlocked) return
    const { active, over } = event
    if (!over || active.id === over.id) return
    ...
    // ── 2. Запоминаем исходный порядок только в начале серии
    if (!pendingReorderRef.current) {
      originalOrderRef.current = curr
    }
    ...
    // ── 4. Сброс дебаунса — 2 секунды с последнего перетаскивания
    clearTimeout(reorderDebounceRef.current)
  ...
  /**
   * flushReorderSave:
   *  Отправляет финальный порядок в базу одним батчем.
   *  Вызывается: автоматически через 2 сек ИЛИ принудительно при нажатии «назад».
   *  После завершения — выходит, если пользователь ждал.
   */
  async function flushReorderSave() {
    const reordered = pendingReorderRef.current
    if (!reordered) { setReorderStatus('idle'); return }
    ...
    try {
      ...
      // Если пользователь ждал выхода — выходим
  ...
  async function handleStatusToggle(task: Task) {
    ...
    if (!ok) { updateTask(task.id, { status: task.status }); toast.error(t('failedStatus')) }
      ...
      const allDone = updated.filter(t => !t.archived).every(t => t.status === 'done')
      if (updated.filter(t => !t.archived).length > 0 && allDone) { setShowConfetti(true); haptic.success() }
    }
  }
  ...
  // ── Индикатор статуса перетаскивания ──────────────────────
    ...
    <div ref={pageRef} className="page-container">
      ...
      <div className="px-4 pt-3 pb-2 flex-shrink-0">
        <div className="flex items-center gap-2 mb-2">
          ...
          <button onClick={toggleTheme} className="btn-ghost p-2" title={theme === 'dark' ? t('light') : t('dark')}>
      ...
      <div className="flex-1 relative min-h-0">
        ...
        <div ref={listRef} className="h-full scrollable px-4 pb-24" onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd}>
          {loading ? (
            <div className="space-y-2 mt-2">
              {[...Array(4)].map((_, i) => <div key={i} className="h-16 skeleton rounded-2xl" style={{ animationDelay: `${i * 80}ms` }} />)}
            </div>
          ) : sorted.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-48 text-center animate-fade-up">
              <p className="text-text-dim text-sm">
            ...
            <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
              <SortableContext items={sorted.map(t => t.id)} strategy={verticalListSortingStrategy}>
                <div className="space-y-2 mt-2">
                  {sorted.map(task => (
                    <SortableTaskCard key={task.id} task={task} isViewer={isViewer} isDndBlocked={isDndBlocked}
                      onToggle={() => handleStatusToggle(task)}
                      onOpen={() => isViewer ? setViewerTask(task) : setActiveTask(task)}
                      onLongPress={(x, y) => { setContextMenu({ task, x, y }); haptic.medium() }} />
                  ))}
      ...
      {/* FAB */}
      {!isViewer && !(typeof window !== 'undefined' && window?.Telegram?.WebApp?.MainButton?.isVisible) && (
        <button onClick={() => { if (isPending) return; setShowCreate(true); haptic.light() }} disabled={isPending}
          className={cn('fixed bottom-6 right-4 w-14 h-14 rounded-2xl bg-accent text-white flex items-center justify-center shadow-glow z-40 active:scale-90 transition-all', isPending && 'opacity-40 cursor-not-allowed')}>
          {isPending ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <Plus size={24} strokeWidth={2.5} />}
        </button>
      )}

      {contextMenu && <ContextMenu items={getContextItems(contextMenu.task)} x={contextMenu.x} y={contextMenu.y} onClose={() => setContextMenu(null)} />}
      {viewerTask  && <ViewerTaskSheet task={viewerTask} userId={user?.id ?? 0} onClose={() => setViewerTask(null)} onSubtaskToggled={() => fetchTasks(false)} />}
      {(showCreate || activeTask) && (
        <TaskSheet listId={activeListId!} userId={user?.id ?? 0} task={activeTask ?? undefined}
          onClose={() => { setShowCreate(false); setActiveTask(null) }}
          onSaved={() => { setShowCreate(false); setActiveTask(null); fetchTasks() }} />
      )}
    </div>
...
interface CardProps {
  task: Task; isViewer: boolean; isDndBlocked: boolean
  onToggle: () => void; onOpen: () => void; onLongPress: (x: number, y: number) => void
}

function SortableTaskCard({ task, isViewer, isDndBlocked, onToggle, onOpen, onLongPress }: CardProps) {
  const { t } = useI18n()
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } =
    useSortable({ id: task.id, disabled: isViewer || isDndBlocked })
  ...
  const priority = PRIORITY_CONFIG[task.priority]
  const isDone = task.status === 'done', isArchived = task.archived
  const subDone = task.subtasks?.filter(s => s.completed).length ?? 0
  const subTotal = task.subtasks?.length ?? 0
  const dueAt = task.due_at ?? task.due_date
  const creatorTz = task.creator_tz ?? 'UTC'
  ...
  return (
    <div ref={setNodeRef} style={style} id={`task-${task.id}`} className="task-item">
      <div className="card-shell">
        <div className={cn('card flex items-start overflow-hidden', isDone && 'opacity-55', isArchived && 'opacity-40', isDragging && 'shadow-glow')}>
          <div className="w-1 self-stretch flex-shrink-0 rounded-l-2xl" style={{ background: priority.dot, opacity: isDone ? 0.4 : 1 }} />
          <div className="flex items-start gap-2.5 p-3.5 flex-1 min-w-0">
            {!isViewer ? (
              <button {...attributes} {...listeners} disabled={isDndBlocked}
                className={cn('text-text-dim mt-0.5 flex-shrink-0 touch-none', isDndBlocked ? 'opacity-25 cursor-not-allowed' : 'cursor-grab active:cursor-grabbing')}>
                <GripVertical size={15} />
              </button>
            ) : (
              <Eye size={13} className="text-text-dim mt-1 flex-shrink-0 opacity-40" />
            )}
            <button onClick={isViewer ? undefined : onToggle}
              className={cn('custom-checkbox mt-0.5', isDone ? 'checked' : 'unchecked', isViewer && 'opacity-60 cursor-default')}>
              {isDone && <svg width="11" height="9" viewBox="0 0 11 9" fill="none"><path d="M1 4L4 7L10 1" stroke="white" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" /></svg>}
            </button>
            <button onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd} onClick={handleClick} className="flex-1 min-w-0 text-left">
              <p className={cn('text-sm font-medium leading-snug', isDone && 'line-through text-text-secondary')}>{task.title}</p>
              {task.description && !isDone && <p className="text-xs text-text-dim mt-0.5 truncate"><MentionText text={task.description} /></p>}
              <div className="flex flex-wrap items-center gap-x-2 gap-y-1 mt-1.5">
                <span className={cn('text-xs px-2 py-0.5 rounded-full font-medium', priority.color, priority.bg)}>{priority.label}</span>
                {task.assigned_user && (
                  <span className="text-xs text-text-secondary flex items-center gap-1">
                    <div className="w-4 h-4 rounded-full bg-accent/20 text-accent flex items-center justify-center text-[9px] font-bold flex-shrink-0">{task.assigned_user.first_name[0]?.toUpperCase()}</div>
                    {task.assigned_user.first_name.split(' ')[0]}
                  </span>
                )}
                {dueAt && (
                  <span className={cn('text-xs flex items-center gap-1', dueOverdue ? 'text-danger' : dueUrgent ? 'text-amber' : 'text-text-secondary')}>
                    <Calendar size={11} />{dueLabel}
                    {dueLocalLabel && <span className="text-accent ml-0.5">· {dueLocalLabel}</span>}
                  </span>
                )}
                {subTotal > 0 && <span className="text-xs text-text-secondary flex items-center gap-1"><CheckCircle2 size={11} />{subDone}/{subTotal}</span>}
                {isArchived && <span className="text-xs text-text-dim">📦 {t('archived')}</span>}
              </div>
            </button>
          </div>
...
function MentionText({ text }: { text: string }) {
  return <>{text.split(/(@[a-zA-Z0-9_]+)/g).map((p, i) => /^@[a-zA-Z0-9_]+$/.test(p) ? <span key={i} className="text-accent font-medium">{p}</span> : <span key={i}>{p}</span>)}</>
}